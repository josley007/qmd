...
export class QMDStore {
        ...
        if (this.rerankContextInstance) {
            try {
                ...
                finalResults = await Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error(`Reranker timed out after ${rerankTimeoutMs}ms`)), rerankTimeoutMs))
                ]);
            }
            catch (err) {
                // Timeout or failure — return RRF-ordered candidates directly (already ranked by fusion score)
                console.warn(`[QMD] Reranker failed/timeout, returning RRF-ordered results:`, err.message);
     ...
     */
    rrfCombine(bm25Results, vecResults, weights, k, limit) {
    ...
    startAutoEmbed(options) {
        ...
        const getCollectionPaths = () => {
            const db = this.getDb();
            const rows = db.prepare('SELECT path FROM collections').all();
